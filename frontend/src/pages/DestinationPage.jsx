...
// Same images as PopularDestinations
const CITY_IMAGES = {
...
function DestinationPage() {
  ...
  const heroImage = CITY_IMAGES[city.toLowerCase()];

  const fetchListings = async (page = 1) => {
    setLoading(true);
    try {
      const result = await axios.get(
        `${serverUrl}/api/listing/getAllListing`,
      ...
      <div className="relative w-full h-[280px] md:h-[360px] mt-[70px] md:mt-[80px]">
        ...
        <div className="absolute bottom-8 left-6 md:left-10">
          ...
          <p className="text-white/70 text-sm mt-1">
            {loading ? "Fetching stays..." : `${listings.length}+ stays available`}
          </p>
        </div>
      </div>
